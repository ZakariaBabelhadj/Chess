...
@onready var turns: Sprite2D = $Turns
...
var white_king_pos: Vector2 = Vector2(0, 4)
var black_king_pos: Vector2 = Vector2(7, 4)

var fifty_move_rule: int = 0

var unique_board_move: Array = []
...
func _input(event: InputEvent) -> void:
	if event is InputEventMouseButton && event.pressed && promoted_square == null:
		if event.button_index == MOUSE_BUTTON_LEFT:
			...
			if !state && (white && board[var2][var1] > 0 || !white && board[var2][var1] < 0):
				...
				show_options()
				state = true
			elif state: set_moves(var2, var1)
...
func _on_button_pressed(button: Button) -> void:
	...
	board[promoted_square.x][promoted_square.y] = -num_char if white else num_char
	white_pieces.visible = false
	black_pieces.visible = false
	promoted_square = null
	display_board()
...
func get_moves(selected : Vector2) -> Array:
	...
	match abs(board[selected.x][selected.y]):
		...
		4:_moves = get_rock_moves(selected)
		5:_moves = get_queen_moves(selected)
		6:_moves = get_king_moves(selected)
		
	return _moves	

func get_rock_moves(piece_position: Vector2) -> Array:
	var _moves = []
	...
	for i in directions:
		...
		while is_valid_position(pos):
			if is_empty(pos):
				...
				if white && !is_in_check(white_king_pos) || !white && !is_in_check(black_king_pos): _moves.append(pos)
				board[pos.x][pos.y] = t
				board[selected_piece.x][selected_piece.y] = 4 if white else -4
				break
			else: break
...
func get_bishop_moves(piece_position: Vector2) -> Array:
	var _moves = []
...
func get_king_moves(piece_position: Vector2) -> Array:
	var _moves = []
	var directions = [Vector2(0, 1), Vector2(0, -1), Vector2(1, 0), Vector2(-1, 0)
	,Vector2(1, 1), Vector2(1, -1), Vector2(-1, 1), Vector2(-1, -1)]
...
func display_board() -> void:
	...
	if white: turns.texture = TURN_WHITE
	else: turns.texture = TURN_BLACK

func promotion(_var: Vector2):
	promoted_square = _var
	white_pieces.visible = white
	black_pieces.visible = !white

func threefold_position(var1: Array):
	for i in unique_board_move.size():
		if var1 == unique_board_move[i]:
			amount_of_same[i] += 1
			if amount_of_same[i] >= 3: print('DRAW')
			return
	unique_board_move.append(var1.duplicate(true))
...
func is_in_check(king_pos: Vector2) -> bool:
	...
	var knight_direction = [Vector2(2, 1), Vector2(2, -1), Vector2(1, 2), Vector2(1, -2),
	Vector2(-2, 1), Vector2(-2, -1), Vector2(-1, 2), Vector2(-1, -2)]
